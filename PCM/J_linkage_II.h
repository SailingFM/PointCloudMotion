...
template<typename T,typename VT, typename CMP>
class fixed_priority_queue: public priority_queue<T,VT,CMP>
{

public:
	fixed_priority_queue(size_t cap):capibility_(cap){}

	void push(  const T& x)
	{
		if (this->size() == capibility_ )
		{
			// 'c' is the container used by priority_queue and is a protected member.
			auto beg = c.begin();
			auto end = c.end();
			auto min = min_element( beg,end, compare_func_ );

			if( compare_func_(*min,x) )
			{
				*min = x;
...
protected:
	...
	struct heap_node
	{
		...
		//bool operator<(const heap_node& rhs){ return dist > rhs.dist; }
...
public:
	J_LinkageAdapter_II( std::vector<Point>& points, std::vector<Line>& lines, std::vector<int>& labels,
								...
								label_set_(labels),p2l_dist_func_(dist_func),min_heap_(30000){  }
	~J_LinkageAdapter_II(){}
	...
	void compute()
	{
		...
		while (true)
		{
			...
			while ( !min_heap_.empty() )
			{
				...
				heap_node min_node = min_heap_.top();
				min_heap_.pop();
...
protected:
	...
	inline void ps_init()
	{
		...
		Logger<<"initial line number of each ps: "<<ps_set_[0]->which_lines.size()<<endl;
	}
	...
	void heap_init()
	{
		...
		//Logger<<sizeof(heap_node)<<std::endl;
		//Logger<<pn<<std::endl;
...
protected:
	...
	fixed_priority_queue< heap_node, std::vector<heap_node>,greater_than >	min_heap_;
