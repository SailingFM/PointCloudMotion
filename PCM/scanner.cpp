...
IndexType Scanner::min_scan_dist_ = 500;
IndexType Scanner::max_scan_dist_ = 2000;
IndexType Scanner::min_frame_ = 100;
IndexType Scanner::max_frame_ = 200;
...
Sample* Scanner::generate_point_cloud( const IndexType sample_idx)
{
	...
	for ( y=0; y < cur_frame_depth_.getHeight(); ++y )
	{
		IndexType idx_shift = y * cur_frame_depth_.getWidth();
		for (x=0; x < cur_frame_depth_.getWidth(); ++x )
		{
