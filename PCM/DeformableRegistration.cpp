...
// 	Sample & tgCloud = m_smpSet[3];
	...
	//global  ICP
	SICP::Parameters pa(false,2,10,1.2,1e5,50,20,1,1e-5);
...
// 	Logger<<resNeigDis<<endl;
	...
	cor.clear_records();
	cor.add_percentCor(0,1,vtx_cor,2,num_vtx);
...
{
	IndexType num_vtx = srGraph.num_vertices() /*distanceField.cols()*/;
...
void DeformableRegistration::calculateMapFunction(Matrix3X & orisrGraphCoor,Sample & srGraph,IndexType axisIndex, Matrix3X & res_mapFunc)
{
	IndexType num_vtx = orisrGraphCoor.cols();
	MatrixXX  Left_mat;
	MatrixXX  Right_mat;
	MatrixXX  Upper_mat;
	MatrixXX  BottomMat;
	...
	constructUpperMat(Upper_mat,orisrGraphCoor,srGraph,axisIndex);

	//assignment 
	Left_mat.block(0,0,num_vtx,num_vtx) = Upper_mat;
	Left_mat.block(0,num_vtx + 1,3,num_vtx) = orisrGraphCoor;
	Left_mat.block(num_vtx + 1,0,num_vtx,3) = orisrGraphCoor.transpose();
	Left_mat.block(num_vtx + 1,num_vtx + 1,4,4) = BottomMat;

	//assignment
	//Right_mat.block(0,0,num_vtx,1) = distanceField.col(axisIndex);
	Right_mat.block(0,0,num_vtx,1) = (srGraph.vertices_matrix() - orisrGraphCoor).col(axisIndex);
	Right_mat.block(num_vtx + 1,0,4,1) = BottomMat.block(0,0,4,1);
...
void DeformableRegistration::constructUpperMat(MatrixXX & upperMat,Matrix3X & oriCoor/*Matrix3X & distanceField*/,Sample & srGraph,IndexType axisIndex)
{
	IndexType num_vtx = srGraph.num_vertices()/*distanceField.cols()*/;
	for (IndexType rowIdx = 0; rowIdx < num_vtx; rowIdx++)
	{
		...
		{
			PointType ori = oriCoor.col(rowIdx);
			PointType cur = srGraph.vertices_matrix().col(colIdx);
			ScalarType len = (cur - ori)[axisIndex];
			ScalarType dis = tpsRBF(len);
			upperMat(rowIdx,colIdx) = dis;
			upperMat(colIdx,rowIdx) = dis;
		}
	}
}
void DeformableRegistration::interpolateCoordinate(Sample & srCloud,Matrix3X & mapFunctionCoff,Sample& tgGraph)
{
	IndexType sr_num_vtx = srCloud.num_vertices();
	IndexType contr_num_vtx = tgGraph.num_vertices();
	MatrixXX  rbfCoff;
      ...
      for (IndexType axisIndex = 0; axisIndex < 3; axisIndex++)
	   {
		   ...
		   rbfCoff.block(contr_num_vtx + 1,0,3,0) = sPoint.transpose();
		   for (IndexType contr_v_it = 0; contr_v_it < contr_num_vtx; contr_v_it ++)
		   {
			   cPoint = tgGraph.vertices_matrix().col(contr_num_vtx);
			   dis = cPoint(0,axisIndex) - sPoint(0,axisIndex);
			   rbfCoff(contr_num_vtx,0)  = tpsRBF(dis);
		   }
...
// 	}
	...
	for (IndexType v_idx = 0; v_idx < num_vtx; v_idx++)
	{
		...
		calculateNeigDis(neigSrCoor,neigDis);//calculate the ori distance around the v_idx
...
// 	}
	...
	PCloudTraj temp;
	IndexType trajNum = 0;
	for (IndexType trajIndex = 0; trajIndex < totsize; trajIndex ++/*= 50*/)
	{
	...
	Logger<<"this length number are =  "<<trajNum<<endl;
}
...
bool DeformableRegistration::sample3validTraj(vector<PCloudTraj> & totTraj,vector<PCloudTraj>& resTraj,LifeSpan & lifesp)
{
 ...
 void DeformableRegistration::intersectionModelWithComTime(PCloudModel & oriModel,LifeSpan & comTime,PCloudModel & newModel)
 {
     ...
     IndexType subId = 0;
	 for (IndexType centerId = comTime.start; centerId <= comTime.end; centerId ++,subId ++)
	 {
		 newModel.Centers.col(subId) = oriModel.Centers.col(centerId);
	 }
 }
 ...
 bool DeformableRegistration::sampleModel(vector<PCloudTraj>& oriTraj,vector<PCloudModel>& smpOriModel,IndexType modelNum)
 {
	   ...
	   PCloudModel resModel;
		...
		// 
		Logger<<"sample size = "<<smpOriModel.size()<<endl;
		return true;
 }
 ...
 void DeformableRegistration::staticDistance(vector<PCloudTraj>& oritrj,vector<PCloudModel>& oriModel)
 {
	 ...
	 ScalarType dis = 0.0;
	 std::fstream op("E:\\1012Traj2modeldisRigid.txt",std::ios::app);
